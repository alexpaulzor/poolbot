...
void reset_to_defaults() {
	...
	set_schedule_item(schedule[0], 9*60,  9*60+15, MODE_SPILL, SPEED_LOW);
	set_schedule_item(schedule[1], 10*60, 10*60,   MODE_CLEAN, SPEED_HI);
	set_schedule_item(schedule[2], 11*60, 15*60,   MODE_POOL,  SPEED_HI);
	set_schedule_item(schedule[3], 18*60, 20*60,   MODE_SPILL, SPEED_MIN);
	set_schedule_item(schedule[4], 21*60, 22*60,   MODE_CLEAN, SPEED_HI);
...
void complete_schedule_item() {
	...
	if (next_item_idx < 0 || next_item.start_time_m == next_item.end_time_m) {
		...
		set_speed(SPEED_OFF);
		time_until = ms_until(get_now_m(), next_item.start_time_m);
		Serial.println("Waiting " + String(time_until/1000l/60l) + "m to item " + 
			String(buf));
		schedule_until = millis() + time_until;
	}
...
int get_next_schedule_item_idx(byte current_idx, int now_m) {
	...
	for (int i = 0; i < SCHED_SLOTS; i++) {
		...
		if (schedule[i].start_time_m != schedule[i].end_time_m &&
				i != current_idx && 
				schedule[i].start_time_m <= now_m && 
				schedule[i].end_time_m >= now_m) {
			// current item
...
long ms_until(int from_m, int to_m) {
	if (to_m < from_m)
		to_m += 24*60;
	return (to_m - from_m) * 60 * 1000l;
}
